class GestorDeUbicaciones:
    ...
    def obtenerNombreMunicipio(self, dni):
        codigoMunicipio = dni[:4]
        for departamento, municipios in self.departamentos.items():
            for municipio, codigo in municipios.items():
                if codigo == codigoMunicipio:
                    return municipio

    def calcularEdad(self, dni):
    ...
    def imprimirResultados(self):
        ...
        municipio = self.obtenerNombreMunicipio(dni)
        edad = self.calcularEdad(dni)
